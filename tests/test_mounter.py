...
def _get_mounter_mixin_for_os(distro):
    if distro == "linux":
        from infi.mount_utils.linux import LinuxMounterMixin
        return LinuxMounterMixin
    elif distro == "solaris":
        from infi.mount_utils.solaris import SolarisMounterMixin
        return SolarisMounterMixin

def _get_mount_repository_mixin_for_os(distro):
    if distro == "linux":
        from infi.mount_utils.linux.mount import LinuxMountRepositoryMixin
        return LinuxMountRepositoryMixin
    elif distro == "solaris":
        from infi.mount_utils.solaris.mount import SolarisMountRepositoryMixin
        return SolarisMountRepositoryMixin

class MounterTestCase(unittest.TestCase):
    @contextmanager
    def patch_execute(self):
        from infi.mount_utils.base import mounter
